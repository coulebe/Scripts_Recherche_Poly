...
% load ca06252cb.mat
dataR = load('car06252cb');
dataL = load('cal06252cb');

AR = dataR.car06252cb(:,1) - 1i*dataR.car06252cb(:,2);
AL = dataL.cal06252cb(:,1) + 1i*dataL.cal06252cb(:,2);
...
AR_dx = zeros(size(AR));
AR_dxx = zeros(size(AR));

for i = 1:t_len
    AR_dx(:,i) = pade_c4(AR(:,i),dx);
    AR_dxx(:,i) = pade_c4(AR_dx(:,i),dx);
end

AR_dt = zeros(size(AR));

for i = 1:x_len
    AR_dt(i,:) = pade_c4(AR(i,:),dt);
end

%% Builds a dictionary matrix

AR_dx = reshape(AR_dx,x_len*t_len,1);
AR_dxx = reshape(AR_dxx,x_len*t_len,1);
AR_dt = reshape(AR_dt,x_len*t_len,1);
AR = reshape(AR,x_len*t_len,1);
AL = reshape(AL,x_len*t_len,1);

theta0 = [AR_dx,AR,AR_dxx,abs(AR).^2.*AR,abs(AL).^2.*AR];
y0 = AR_dt;

%% Number of snapshots: 108000
...
rng('default');
rng(0);
index = randperm(length(index),length(index));
theta = theta(index,:);
y = y(index);

%4/5 for training
theta_train = theta(1:0.8*length(index),:);
y_train = y(1:0.8*length(index));
theta_train = [real(theta_train) -imag(theta_train(:,2:5));imag(theta_train) real(theta_train(:,2:5))];
y_train = [real(y_train);imag(y_train)];

%1/5 for validation
theta_val = theta(0.8*length(index)+1:end,:);
y_val = y(0.8*length(index)+1:end);
