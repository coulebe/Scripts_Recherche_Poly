...
# In this notebook, we'll discover model of EWH1 (OSO300L) form the TB datat in MoD1, i.e., we have Temperature, ambiant temp,  draw debit(m³/s), and power consumption(W)
#The dataset values is normalized
# In[2]:
...
for file_name in files:
    ...
    network = NN(4, [100, 100, 100, 100], 1)


    # Configuration of the library function: We select the custom build library we created earlier
    # * [$1, T, \frac{\partial^{2} T}{\partial x^{2}}, V \frac{\partial T}{\partial x}, P$] 

    # 


    Library = DD_func.Library_MoD1_Norm()
    ...
    train(
        ...
        optimizer,
        sparsity_scheduler,
        log_dir= os.getcwd() + "\\Logdir\\MoD1_Norm\\" ,
    ...
    result.loc[test_name, "1"] = (T_max-T_min) * (coeff_norm[0] - coeff_norm[1]*((T_min/(T_max-T_min)) ) - (coeff_norm[4]*P_min/(P_max-P_min)) ) / (t_max-t_min)
    result.loc[test_name,"T"] = coeff_norm[1] / (t_max-t_min)
    result.loc[test_name,"Ta"] = -result.loc[test_name, "1"]/result.loc[test_name,"T"]# Minus added bcause we have smthg(T-Ta)
    ...
    prev = pd.read_csv("MoD1_results_Norm.csv", index_col=0)
    result = pd.concat([prev, result])
    result.to_csv("MoD1_results_Norm.csv")

    #Delete logdir generated files(can't replace it by None in the train function :/)
    DD_func.remove(os.getcwd() + "\\Logdir\\MoD1_Norm\\")
